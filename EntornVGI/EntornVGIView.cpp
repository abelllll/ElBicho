...
BEGIN_MESSAGE_MAP(CEntornVGIView, CView)
	ON_COMMAND(ID_ILUMINACIO_TEXTURA_BRICKS, &CEntornVGIView::OnIluminacioTexturaBricks)
	ON_COMMAND(ID_ILUMINACIO_TEXTURA_MAPAMUNDI, &CEntornVGIView::OnIluminacioTexturaMapamundi)
	ON_COMMAND(ID_ILUMINACIO_TEXTURA_SAC, &CEntornVGIView::OnIluminacioTexturaSac)
...
// Construcción o destrucción de CEntornVGIView
	void CEntornVGIView::OnIluminacioTexturaBricks()
	{
		t_textura = TEXTURA_BRICKS;
		CarregaTexturaSegonsTipus();
		Invalidate(FALSE);  // Redibuixar la vista
	}

	void CEntornVGIView::OnIluminacioTexturaMapamundi()
	{
		t_textura = TEXTURA_MAPA_MUNDI;
		CarregaTexturaSegonsTipus();
		Invalidate(FALSE);
	}

	void CEntornVGIView::OnIluminacioTexturaSac()
	{
		t_textura = TEXTURA_SAC;
		CarregaTexturaSegonsTipus();
		Invalidate(FALSE);
	}
	void CEntornVGIView::CarregaTexturaSegonsTipus()
	{
		// Abrimos contexto OpenGL
		wglMakeCurrent(m_pDC->GetSafeHdc(), m_hrc);

		// Borramos texturas previamente cargadas
		...
		// --- Cargar la textura según t_textura ---
		switch (t_textura)
		{
		case TEXTURA_BRICKS:
			texturesID[0] = loadIMA_SOIL((char*)"../Release/textures/brick.jpg");
			break;

		case TEXTURA_MAPA_MUNDI:
			texturesID[0] = loadIMA_SOIL((char*)"../Release/textures/world-political-map-with-countries.JPG");
			// si luego se ve patas arriba, ponemos tFlag_invert_Y = true;
			tFlag_invert_Y = true;
			break;

		case TEXTURA_SAC:
			texturesID[0] = loadIMA_SOIL((char*)"../Release/textures/sac.JPG");
			break;
		...
		// --- DEBUG: comprobar si ha cargado ---
		if (!texturesID[0] && t_textura != TEXTURA_CAP)
		{
			AfxMessageBox(L"No s'ha pogut carregar la textura (texturesID[0] = 0)!");
		}

		// --- Activar/Desactivar flags de textura ---
		textura = (t_textura != TEXTURA_CAP);
		textura_map = textura;

		// --- Vincular la textura a la unitat 0 ---
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texturesID[0]);

		// Pasar parámetros al shader
		glUniform1i(glGetUniformLocation(shader_programID, "texture0"), 0);              // unitat 0
...
// Entorn VGI: FPS
	beginFrame = 0;	endFrame = 0;	deltaTime = 0;
	frames = 0;
	frameRate = 30;	averageFrameTimeMilliseconds = 33.333;
	fps = 0;
...
// EntornVGI: FPS
	beginFrame = clock();	// Iniciar Rellotge per a FPS.
...
// seleccionat una projecció o un altra
	// Declaramos alt una sola vez antes del switch
	int alt = 0;
	...
	case AXONOM: {
		...
		// Calcular el tamaño de la ventana (ample, alt)
		CRect rect;
		GetClientRect(&rect);
		int ample = rect.Width();
		alt = rect.Height(); // solo asignación
		...
		// Aquí farem les crides per a definir Viewport, Projecció i Càmara: FI -------------------------
	...
	case ORTO: {
		// 1) Activar recorte y limpiar TODO el fondo una sola vez
		glEnable(GL_SCISSOR_TEST);
		glViewport(0, 0, w, h);
		glScissor(0, 0, w, h);
		...
		// 2) Medidas EXACTAS de los cuatro cuadrantes (mitades perfectas)
		const int halfW = w / 2;
		const int halfH = h / 2;

		// --- guardar y neutralizar PAN/traslaciones globales en ORTO ---
		bool    pan_bk = pan;
		CPunt3D tr_bk = tr_cpv;
		CPunt3D trF_bk = tr_cpvF;
		pan = false;
		tr_cpv.x = tr_cpv.y = tr_cpv.z = 0.0f;
		tr_cpvF.x = tr_cpvF.y = tr_cpvF.z = 0.0f;
		// ----------------------------------------------------------------

		// ---------- BOTTOM-LEFT (PLANTA) ----------
		ProjectionMatrix = Projeccio_Orto(shader_programID, 0, 0, halfW, halfH);
		glClear(GL_DEPTH_BUFFER_BIT);
		ViewMatrix = Vista_Ortografica(shader_programID, PLANTA, OPV.R, c_fons, col_obj, objecte, mida, pas,
			front_faces, oculta, test_vis, ilumina, llum_ambient, llumGL, ifixe, ilum2sides,
			eixos, grid, hgrid);
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "projection"), 1, GL_FALSE, glm::value_ptr(ProjectionMatrix));
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "view"), 1, GL_FALSE, glm::value_ptr(ViewMatrix));
		configura_Escena(); dibuixa_Escena();

		// ---------- BOTTOM-RIGHT (ISO) ----------
		ProjectionMatrix = Projeccio_Orto(shader_programID, halfW, 0, halfW, halfH);
		glClear(GL_DEPTH_BUFFER_BIT);
		ViewMatrix = Vista_Ortografica(shader_programID, ISO, OPV.R, c_fons, col_obj, objecte, mida, pas,
			front_faces, oculta, test_vis, ilumina, llum_ambient, llumGL, ifixe, ilum2sides,
			eixos, grid, hgrid);
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "projection"), 1, GL_FALSE, glm::value_ptr(ProjectionMatrix));
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "view"), 1, GL_FALSE, glm::value_ptr(ViewMatrix));
		...
		// ---------- TOP-LEFT (ALÇAT) ----------
		ProjectionMatrix = Projeccio_Orto(shader_programID, 0, halfH, halfW, halfH);
		glClear(GL_DEPTH_BUFFER_BIT);
		ViewMatrix = Vista_Ortografica(shader_programID, ALÇAT, OPV.R, c_fons, col_obj, objecte, mida, pas,
			front_faces, oculta, test_vis, ilumina, llum_ambient, llumGL, ifixe, ilum2sides,
			eixos, grid, hgrid);
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "projection"), 1, GL_FALSE, glm::value_ptr(ProjectionMatrix));
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "view"), 1, GL_FALSE, glm::value_ptr(ViewMatrix));
		...
		// ---------- TOP-RIGHT (PERFIL) ----------
		ProjectionMatrix = Projeccio_Orto(shader_programID, halfW, halfH, halfW, halfH);
		glClear(GL_DEPTH_BUFFER_BIT);
		ViewMatrix = Vista_Ortografica(shader_programID, PERFIL, OPV.R, c_fons, col_obj, objecte, mida, pas,
			front_faces, oculta, test_vis, ilumina, llum_ambient, llumGL, ifixe, ilum2sides,
			eixos, grid, hgrid);
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "projection"), 1, GL_FALSE, glm::value_ptr(ProjectionMatrix));
		glUniformMatrix4fv(glGetUniformLocation(shader_programID, "view"), 1, GL_FALSE, glm::value_ptr(ViewMatrix));
	...
	default: {
		...
		// Entorn VGI: Creació de la llista que dibuixarà els eixos Coordenades Món
		if (!eixos_Id)
			eixos_Id = deixos();
...
// EntornVGI: FPS
	...
	if ((deltaTime / (double)CLOCKS_PER_SEC) * 1000.0 > 1000.0)
	{ //every second
		frameRate = (double)frames * 0.5 + frameRate * 0.5; //more stable
		frames = 0;
		deltaTime -= CLOCKS_PER_SEC;
		averageFrameTimeMilliseconds = 1000.0 / (frameRate == 0 ? 0.001 : frameRate);
		fps = (frameRate == 0 ? 0.001 : frameRate);
	}
...
// Refrescar Pas de dibuix del fractal en Status Bar
	GetStatusBar().SetPaneText(10, sss);

	//buffer.Format(_T("%.2f"), averageFrameTimeMilliseconds);	// Milisegons per a cada frama
	//sss = _T("t=") + buffer;
	buffer.Format(_T("%.2f"), fps);	// Nombre de frames per segon (FPS)
	sss = _T("fps=") + buffer;

// Refrescar FPS en Status Bar
	GetStatusBar().SetPaneText(11, sss);
}
...
// Entorn VGI: Obrir diàleg de lectura de fitxer (fitxers (*.MNT)
	...
	if (openMunt.DoModal() != IDOK) return;	// stay with old data file
	else {
...
void CEntornVGIView::OnObjecteCub()
{
	...
	// --- Càlcul de diàmetre i distància de càmera (perspectiva)
	// Cub d'aresta 5 → diagonal D = 5*sqrt(3)
	double diag = 5.0 * std::sqrt(3.0);
	double factor = 1.5;      // marge f entre 1 i 1.5
	mida = diag;              // guardem el diàmetre real
	OPV.R = factor * diag;    // distància de la càmera en perspectiva

	// (resto de código ya existente)
	wglMakeCurrent(m_pDC->GetSafeHdc(), m_hrc);
	netejaVAOList();
...
// TODO: Agregue aquí su código de controlador de comandos
	...
	const double diag = 5.0 * std::sqrt(3.0);   // cubo de arista 5 ⇒ diagonal
	const double factor = 1.5;                    // margen (1 ≤ f ≤ 1.5)
	mida = diag;                                 // guardamos el diámetro
	OPV.R = factor * diag;
...
// TODO: Agregue aquí su código de controlador de comandos
	...
	const double diag = 10.0;   // esfera de radio 5 ⇒ diámetro 10
...
// TODO: Agregue aquí su código de controlador de comandos
	...
	const double diag = 4.2;    // aproximación: diámetro ≈ 4 (radio ≈ 2)
...
{
	projeccio = ORTO;       // Activa proyección ortográfica
	mobil = false;       // Desactiva movimiento interactivo en ortho
	zzoom = false;       // Desactiva zoom interactivo en ortho
	InvalidateRect(NULL, false); // Redibuja la escena
}

void CEntornVGIView::OnUpdateProjeccioOrtografica(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(projeccio == ORTO); // Activa el tick si está activa
}
